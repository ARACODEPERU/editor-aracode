const G = {
  es: {
    ...
    pasteImageUploading: "Subiendo imagen…",
    pasteImageError: "No se pudo subir la imagen",
    pasteImageMissing: "Imagen no disponible",
  ...
  en: {
    ...
    pasteImageUploading: "Uploading image…",
    pasteImageError: "Could not upload image",
    pasteImageMissing: "Image unavailable",
...
function g(i, e = "es") {
  var t;
  return ((t = G[e]) == null ? void 0 : t[i]) || G.es[i] || i;
}
const J = 10, I = 50;
class me {
  constructor(e, t) {
    this.editor = e, this.onInsert = t, this.overlay = null, this.dialog = null, this._outsideHandler = null, this._keyHandler = null, this._headerRow = !1, this._savedRange = null;
  }
  toggle() {
    if (this.overlay) {
      this.close();
    ...
    this._savedRange = this._saveSelection(), this._render();
  }
  close() {
    var e;
    this._keyHandler && (document.removeEventListener("keydown", this._keyHandler), this._keyHandler = null), this._outsideHandler && (document.removeEventListener("mousedown", this._outsideHandler, !0), this._outsideHandler = null), (e = this.overlay) != null && e.parentNode && this.overlay.parentNode.removeChild(this.overlay), this.overlay = null, this.dialog = null, this._otherModalOpen() || (document.body.style.overflow = "");
  }
  _otherModalOpen() {
  ...
  _saveSelection() {
    ...
    return this.editor.editable.contains(t.commonAncestorContainer) ? t.cloneRange() : null;
  }
  _restoreSelection() {
    if (!this._savedRange) return;
    const e = window.getSelection();
    e.removeAllRanges(), e.addRange(this._savedRange);
  }
  _clampSize(e) {
    const t = parseInt(e, 10);
    return Number.isFinite(t) ? Math.max(1, Math.min(I, t)) : 1;
  }
  _render() {
    const e = this.editor.options.locale;
    document.body.style.overflow = "hidden";
    const t = document.createElement("div");
    t.className = "aracode-dialog-overlay aracode-table-picker-overlay", t.setAttribute("role", "presentation");
    const n = document.createElement("div");
    n.className = "aracode-dialog aracode-table-picker-dialog", n.setAttribute("role", "dialog"), n.setAttribute("aria-modal", "true"), n.setAttribute("aria-label", g("table", e));
    const a = document.createElement("div");
    a.className = "aracode-dialog-header", a.textContent = g("table", e);
    const o = document.createElement("button");
    o.type = "button", o.className = "aracode-dialog-close", o.setAttribute("aria-label", g("cancel", e)), o.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>', o.addEventListener("mousedown", (y) => y.preventDefault()), o.addEventListener("click", () => this.close()), a.appendChild(o);
    const r = document.createElement("div");
    r.className = "aracode-dialog-body aracode-table-picker-body";
    const s = document.createElement("div");
    s.className = "aracode-table-picker-label", s.textContent = g("tablePickerHint", e);
    ...
    p.type = "number", p.className = "aracode-table-picker-input", p.min = "1", p.max = String(I), p.value = "3", u.appendChild(p);
    ...
    v.type = "button", v.className = "aracode-btn aracode-btn-secondary", v.textContent = g("cancel", e), v.addEventListener("mousedown", (y) => y.preventDefault()), v.addEventListener("click", () => this.close()), E.appendChild(v), n.appendChild(a), n.appendChild(r), n.appendChild(E), t.appendChild(n), document.body.appendChild(t), this.overlay = t, this.dialog = n, t.addEventListener("click", (y) => {
      y.target === t && this.close();
    }), this._keyHandler = (y) => {
      y.key === "Escape" && this.close();
    }, document.addEventListener("keydown", this._keyHandler), setTimeout(() => n.classList.add("is-open"), 10);
  }
  _setHover(e, t, n, a, o) {
    a.textContent = g("tablePickerSize", o).replace("{rows}", e).replace("{cols}", t), n.querySelectorAll(".aracode-table-picker-cell").forEach((r) => {
      const s = parseInt(r.dataset.row, 10), l = parseInt(r.dataset.col, 10);
      r.classList.toggle("is-highlighted", s <= e && l <= t);
    });
  }
  _insert(e, t) {
    this._restoreSelection(), this.onInsert(e, t, this._headerRow, this._savedRange), this.close();
  }
}
const Y = [
  "bold",
...
class ve {
  ...
  build() {
    const e = this.editor.options.toolbar || Y;
    let t = document.createElement("div");
    t.className = "aracode-toolbar-group", e.forEach((n, a) => {
      if (n === "|") {
        this.container.appendChild(t);
        const r = document.createElement("span");
        r.className = "aracode-toolbar-separator", this.container.appendChild(r), t = document.createElement("div"), t.className = "aracode-toolbar-group";
        return;
      }
      if (n === "heading") {
      ...
      if (n === "textColor" || n === "backgroundColor") {
        ...
        }), t.appendChild(r), this.buttons[n] = r;
        return;
      }
      const o = document.createElement("button");
  ...
  handleToolClick(e) {
    ...
    if (["alignLeft", "alignCenter", "alignRight"].includes(e) && ((n = this.editor.imageController) == null ? void 0 : n.selectedImage)) {
  ...
  toggleFontPicker(e) {
    ...
    r.className = "aracode-font-search", r.type = "search", r.placeholder = g("fontSearchPlaceholder", this.editor.options.locale);
    const s = document.createElement("div");
    ...
    const l = () => {
      ...
      s.innerHTML = "", h.forEach((u) => {
        ...
        p.type = "button", p.className = "aracode-font-option", p.textContent = u, p.style.fontFamily = `"${u}", sans-serif`, p.addEventListener("mousedown", (b) => b.preventDefault()), p.addEventListener("click", () => {
          if (a) {
            const b = window.getSelection();
            b.removeAllRanges(), b.addRange(a);
          }
          this.editor.loadFontFamily(u), this.editor.commands.fontFamily(u), o.remove();
        }), s.appendChild(p);
      });
    };
  ...
  updateActiveStates() {
    (this.editor.options.toolbar || Y).forEach((t) => {
      ...
      if (a)
        try {
          o = document.queryCommandState(a);
        } catch {
          o = !1;
        }
      else t === "sourceView" ? o = this.editor._isSourceView : t === "fullscreen" && (o = this.editor._isFullscreen);
...
class ye {
  ...
  exec(e, t = null) {
    const n = this.editor.editable, a = this.editor.consumeToolbarSelection();
    n.focus(), a && this.editor.restoreToolbarSelection(a), document.execCommand(e, !1, t), this.editor.emit("change", this.editor.getHTML()), this.editor.emit("command", e, t);
  }
  bold() {
    this.exec("bold");
  }
  ...
  _toggleList(e) {
    const t = this.editor.editable, n = e === "ol" ? "insertOrderedList" : "insertUnorderedList", a = this.editor.consumeToolbarSelection();
    t.focus(), a && this.editor.restoreToolbarSelection(a);
    const o = t.innerHTML;
    document.execCommand(n, !1, null), (!this._selectionInList(e) || t.innerHTML === o) && this._insertListFallback(e, a), this.editor.emit("change", this.editor.getHTML()), this.editor.emit("command", n);
  }
  _selectionInList(e) {
    var a;
    const t = window.getSelection();
    if (!(t != null && t.rangeCount)) return !1;
    let n = t.anchorNode;
    ...
    if (r && n.contains(r)) {
      if (r.tagName.toLowerCase() === e) {
        ...
        h.forEach((p) => {
          ...
          b.innerHTML = p.innerHTML || "<br>", u.insertBefore(b, r);
        }), r.remove();
      } else {
        const h = document.createElement(e);
        h.innerHTML = r.innerHTML, r.replaceWith(h);
      }
      return;
    }
  ...
  _appendEmptyList(e) {
    const t = this.editor.editable, n = document.createElement(e), a = document.createElement("li");
    a.innerHTML = "<br>", n.appendChild(a), t.appendChild(n), this._placeCaretIn(a);
  }
  _placeCaretIn(e) {
  ...
  code() {
    const n = (window.getSelection().toString() || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    this.exec("insertHTML", `<code>${n}</code>`);
  }
  removeFormat() {
  ...
  align(e) {
    var n, a;
    const t = (n = this.editor.imageController) == null ? void 0 : n.selectedImage;
    t && (t.className = e, (a = this.editor.imageController) != null && a.container && (this.editor.imageController.container.classList.remove("align-left", "align-center", "align-right"), e && this.editor.imageController.container.classList.add(e)), this.editor.emit("change", this.editor.getHTML()));
  }
  alignLeft() {
    var e;
    (e = this.editor.imageController) != null && e.selectedImage ? this.align("align-left") : this.exec("justifyLeft");
  }
  alignCenter() {
    var e;
    (e = this.editor.imageController) != null && e.selectedImage ? this.align("align-center") : this.exec("justifyCenter");
  }
  alignRight() {
    var e;
    (e = this.editor.imageController) != null && e.selectedImage ? this.align("align-right") : this.exec("justifyRight");
  }
  alignJustify() {
  ...
  codeBlock() {
    const n = (window.getSelection().toString() || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    this.exec("insertHTML", `<pre><code>${n}</code></pre>`);
  }
  createLink(e, t, n = "") {
    var r, s;
    const a = this.editor.editable;
    a.focus();
  ...
  fontFamily(e) {
    ...
    this.editor.editable.contains(n.commonAncestorContainer) && (this.editor.editable.focus(), document.execCommand("fontName", !1, e), this.editor.editable.querySelectorAll("font[face]").forEach((a) => {
  ...
  backgroundColor(e) {
    this.editor.editable.focus(), document.execCommand("styleWithCSS", !1, !0);
    const n = e || "transparent";
    document.execCommand("hiliteColor", !1, n) || document.execCommand("backColor", !1, n), this.editor.emit("change", this.editor.getHTML()), this.editor.emit("command", "hiliteColor", e);
  }
  indent() {
    this.exec("indent");
  }
  ...
  insertTable(e, t, n = !1, a = null) {
    const o = Math.max(1, Math.min(I, parseInt(e, 10) || 1)), r = Math.max(1, Math.min(I, parseInt(t, 10) || 1)), s = this.editor.editable;
    let l = '<table class="aracode-table"><tbody>';
    for (let p = 0; p < o; p++) {
      l += "<tr>";
      for (let b = 0; b < r; b++) {
        const w = n && p === 0 ? "th" : "td";
...
function Ce(i, e, t) {
  ...
  let n = e;
  const a = document.createElement("div");
  a.className = "aracode-dialog-dropzone-countdown", a.textContent = n, i.appendChild(a), i._countdownTimer = setInterval(() => {
    n -= 1, a.textContent = n, n <= 0 && (clearInterval(i._countdownTimer), i._countdownTimer = null, setTimeout(() => {
      typeof t == "function" && t();
    }, 250));
  }, 1e3);
}
function xe(i, e, t, n) {
  ...
  o.className = "aracode-dialog-dropzone", o.innerHTML = `
    <div class="aracode-dialog-dropzone-inner">
      ...
      <div class="aracode-dialog-dropzone-text">${n || "Haz clic o arrastra un archivo"}</div>
    </div>
  ...
  }), o.addEventListener("dragleave", () => o.classList.remove("is-dragover")), o.addEventListener("drop", (s) => {
...
class Ee {
  ...
  open(e, t, n) {
    ...
    const o = document.createElement("div");
    o.className = "aracode-dialog-header", o.textContent = e;
    const r = document.createElement("button");
    r.className = "aracode-dialog-close", r.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>', r.addEventListener("click", () => this.close()), o.appendChild(r);
    const s = document.createElement("div");
    ...
    function w(m, E) {
        ...
        f.className = `aracode-btn ${m.primary ? "aracode-btn-primary" : "aracode-btn-secondary"}`, f.textContent = m.label, f.style.marginTop = "12px", f.style.width = "100%", f.addEventListener("click", (x) => {
          if (x.preventDefault(), m.onClick) {
            ...
            for (const k of Object.keys(a)) {
              const L = a[k];
              L instanceof HTMLInputElement || L instanceof HTMLSelectElement ? C[k] = L.type === "checkbox" ? L.checked : L.value : L && typeof L == "object" && "value" in L && (C[k] = L.value);
            }
            m.onClick(C, b);
          }
        }), E.appendChild(f);
      ...
      if (m.type === "image-preview") {
        const f = document.createElement("img");
        f.className = "aracode-dialog-url-preview", f.style.cssText = "max-width:100%;max-height:150px;display:none;margin:10px auto;border-radius:4px;object-fit:contain;", E.appendChild(f);
        return;
      }
      if (m.type === "select") {
        const f = document.createElement("div");
        if (f.className = "aracode-dialog-field", m.label) {
      ...
      if (v.className = "aracode-dialog-field", m.label) {
        const f = document.createElement("label");
        f.className = "aracode-dialog-label", f.textContent = m.label, v.appendChild(f);
      }
  ...
  switchTab(e) {
    ...
    const t = this.dialog.querySelectorAll(".aracode-dialog-tab-btn"), n = this.dialog.querySelectorAll(".aracode-dialog-tab-content"), a = this.dialog.querySelector(".aracode-dialog-apply-btn"), o = this.dialog.querySelector(".aracode-dialog-upload-btn");
    t[e] && n[e] && (t.forEach((r) => r.classList.remove("is-active")), n.forEach((r) => r.classList.remove("is-active")), t[e].classList.add("is-active"), n[e].classList.add("is-active"), o && (o.style.display = e === 0 ? "" : "none"), a && (a.style.display = e === 1 ? "" : "none"));
  }
  updateProgress(e) {
    if (!this.dialog) return;
    const t = this.dialog.querySelector(".aracode-dialog-progress-bar"), n = this.dialog.querySelector(".aracode-dialog-progress");
    t && n && (n.style.display = "block", t.style.width = e + "%");
  }
  setValue(e, t) {
    if (!this.dialog) return;
    const n = this.dialog.querySelector(`[name="${e}"]`);
    n && (n.value = t, n.dispatchEvent(new Event("input", { bubbles: !0 })));
  }
  close() {
...
class Le {
  ...
  build() {
    ...
    t.type = "button", t.className = "aracode-color-picker-remove", t.textContent = "✕ Sin color", t.addEventListener("mousedown", (n) => n.preventDefault()), t.addEventListener("click", (n) => {
      n.preventDefault(), n.stopPropagation(), this.onSelect(""), this.hide();
    }), this.container.appendChild(e), this.container.appendChild(t), document.body.appendChild(this.container);
  }
  isOpenFor(e) {
    return this._anchor === e && this.container.classList.contains("is-visible");
  }
  show(e) {
    this._anchor = e;
    const t = e.getBoundingClientRect();
    this.container.style.top = `${t.bottom + 4}px`, this.container.style.left = `${Math.max(8, t.left)}px`, this.container.classList.add("is-visible"), this._outsideHandler && document.removeEventListener("mousedown", this._outsideHandler, !0), this._outsideHandler = (n) => {
      this.container.contains(n.target) || e.contains(n.target) || this.hide();
    }, setTimeout(() => {
      document.addEventListener("mousedown", this._outsideHandler, !0);
    }, 0);
  }
  hide() {
    this.container.classList.remove("is-visible"), this._anchor = null, this._outsideHandler && (document.removeEventListener("mousedown", this._outsideHandler, !0), this._outsideHandler = null);
  }
  destroy() {
...
class Te {
  ...
  selectImage(e) {
    ...
    const t = ["align-left", "align-center", "align-right"].find((a) => e.classList.contains(a));
    this.container = document.createElement("span"), this.container.className = "aracode-img-container", t && this.container.classList.add(t), e.parentNode.insertBefore(this.container, e), this.container.appendChild(e), ["tl", "tr", "bl", "br"].forEach((a) => {
      const o = document.createElement("div");
      o.className = `aracode-img-resizer ${a}`, o.addEventListener("mousedown", (r) => this._onResizeStart(r, a)), this.container.appendChild(o);
    }), this._initDrag();
  }
  deselectImage() {
    if (!this.selectedImage || !this.container) return;
    const e = this.selectedImage, t = ["align-left", "align-center", "align-right"].find((n) => this.container.classList.contains(n));
    e.classList.remove("align-left", "align-center", "align-right"), t && e.classList.add(t), this.container.parentNode && (this.container.parentNode.insertBefore(e, this.container), this.container.parentNode.removeChild(this.container)), this.selectedImage = null, this.container = null, this._removeDropIndicator();
  }
  _initDrag() {
    if (!this.container) return;
    const e = (a) => {
      a.target.closest(".aracode-img-resizer") || (a.preventDefault(), this._dragStartPos = { x: a.clientX, y: a.clientY }, this._isDragging = !1, document.addEventListener("mousemove", t), document.addEventListener("mouseup", n));
    }, t = (a) => {
      if (!this._dragStartPos) return;
      const o = a.clientX - this._dragStartPos.x, r = a.clientY - this._dragStartPos.y;
      (Math.abs(o) > 5 || Math.abs(r) > 5) && (this._isDragging || (this._isDragging = !0, this.container.style.opacity = "0.4", this._createDropIndicator()), this._updateDropPosition(a.clientX, a.clientY));
    }, n = (a) => {
      document.removeEventListener("mousemove", t), document.removeEventListener("mouseup", n), this._isDragging && this._finishDrag(a.clientX, a.clientY), this._dragStartPos = null, this._isDragging = !1;
    };
    this.container.addEventListener("mousedown", e);
  }
  _createDropIndicator() {
    this._removeDropIndicator(), this._dropIndicator = document.createElement("div"), this._dropIndicator.className = "aracode-drop-indicator", this.editor.editable.appendChild(this._dropIndicator);
  }
  _removeDropIndicator() {
    this._dropIndicator && this._dropIndicator.parentNode && this._dropIndicator.parentNode.removeChild(this._dropIndicator), this._dropIndicator = null;
  }
  _updateDropPosition(e, t) {
    ...
    const a = n.getClientRects()[0];
    if (a) {
      const o = this.editor.editable.getBoundingClientRect();
      this._dropIndicator.style.top = a.top - o.top + "px", this._dropIndicator.style.left = a.left - o.left + "px", this._dropIndicator.style.height = a.height + "px";
    }
  }
  _getDropRange(e, t) {
    const n = this.editor.editable;
    if (!document.caretRangeFromPoint) return null;
    const a = document.caretRangeFromPoint(e, t);
    return !a || !n.contains(a.commonAncestorContainer) ? null : a;
  }
  _finishDrag(e, t) {
...
function _e(i, e) {
  ...
  for (let s = 0; s < o; s++)
    a.appendChild(M(ne(t, s)));
  t.parentNode.insertBefore(a, t), e.emit("change", e.getHTML());
...
function Se(i, e) {
  ...
  for (let s = 0; s < o; s++)
    a.appendChild(M(ne(t, s)));
  t.parentNode.insertBefore(a, t.nextSibling), e.emit("change", e.getHTML());
  const r = a.cells[i.cellIndex] || a.cells[0];
  return _(r, e.editable), r;
}
function Ae(i, e) {
...
function Ne(i, e) {
  ...
  const o = a.indexOf(t), r = a[o + 1] || a[o - 1], s = i.cellIndex;
  t.remove(), e.emit("change", e.getHTML());
...
function Ie(i, e) {
  ...
  if (!t) return null;
  const n = U(t);
  if ((((s = n[0]) == null ? void 0 : s.cells.length) || 0) <= 1) return null;
  const o = i.cellIndex;
  let r = null;
  return n.forEach((l) => {
...
function Me(i, e) {
  ...
  if (n) {
    ...
    s.innerHTML = "<br>", o.insertBefore(s, n), _(s, o);
    return;
  }
  if (a) {
    o.focus();
...
class De {
  ...
  _bindEvents() {
    this.editor.editable.addEventListener("keydown", (e) => {
      if (e.key !== "Tab") return;
      const t = this._getActiveCell();
      if (!t) return;
      e.preventDefault();
      const n = e.shiftKey ? this._findPreviousCell(t) : this._findNextCell(t);
      n && this._focusCell(n);
    }), this.editor.editable.addEventListener("contextmenu", (e) => {
      const t = this._getCellFromTarget(e.target);
      t && (e.preventDefault(), this.contextMenu.open(t, e.clientX, e.clientY));
    });
  }
  _getCellFromTarget(e) {
    var o;
    if (!e) return null;
    const t = e.nodeType === Node.TEXT_NODE ? e.parentElement : e, n = (o = t == null ? void 0 : t.closest) == null ? void 0 : o.call(t, "td, th");
    if (!n) return null;
    const a = n.closest("table.aracode-table, table");
    return !a || !this.editor.editable.contains(a) ? null : n;
  }
  _getActiveCell() {
    const e = window.getSelection();
    if (!e.rangeCount) return null;
    let t = e.anchorNode;
    return (t == null ? void 0 : t.nodeType) === Node.TEXT_NODE && (t = t.parentElement), this._getCellFromTarget(t);
  }
  _getTableRows(e) {
  ...
  _findNextCell(e) {
    ...
    if (!n || !t) return null;
    const a = this._getTableRows(n), o = a.indexOf(t), r = Array.from(t.cells), s = r.indexOf(e);
  ...
  _findPreviousCell(e) {
    ...
    if (o > 0) {
      const l = a[o - 1];
      return l.cells[l.cells.length - 1] || null;
    }
    return e;
  }
  _focusCell(e) {
    ...
    const n = window.getSelection();
    n.removeAllRanges(), n.addRange(t), this.editor.editable.focus();
  }
  destroy() {
...
function ae(i, e) {
  const t = document.createRange();
  return t.setStart(e.startContainer, e.startOffset), t.setEnd(i, i.childNodes.length), t.toString();
}
function Fe(i, e) {
  const t = document.createRange();
...
`);
  ...
  for (; o.nextNode(); ) {
    ...
    if (l === n && l.nodeType === Node.TEXT_NODE) {
      r += l.textContent.slice(0, a), s = !0;
      break;
    }
    if (l.nodeName === "BR") {
      r = "";
...
function Ue(i) {
  for (; i.lastChild; ) {
    ...
    if (e.nodeType === Node.TEXT_NODE && !R(e.textContent)) {
      i.removeChild(e);
      continue;
    }
    if (e.nodeType === Node.ELEMENT_NODE && e.nodeName !== "BR" && D(e)) {
      i.removeChild(e);
      continue;
    }
    break;
...
function Q(i, e) {
  ...
  for (; t && t.tagName === "BLOCKQUOTE" && D(t); ) {
    const n = t;
...
function ie(i) {
  ...
  return e != null && e.length ? btoa(e.map((t) => String.fromCharCode(parseInt(t, 16))).join("")) : "";
}
function oe(i) {
  if (!i) return [];
  const e = /{\\pict[\s\S]+?\\bliptag-?\d+(\\blipupi-?\d+)?({\\\*\\blipuid\s?[\da-fA-F]+)?[\s}]*?/, t = new RegExp(
    `(?:(${e.source}))([\\da-fA-F\\s]+)\\}`,
    "g"
  ), n = i.match(t), a = [];
  if (!n) return a;
  for (const o of n) {
    let r = null;
    o.includes("\\pngblip") ? r = "image/png" : o.includes("\\jpegblip") && (r = "image/jpeg"), r && a.push({
...
function We(i, e) {
  ...
  for (let o = 0; o < a; o += 1) {
    const { hex: r, type: s } = n[o];
    if (r)
      try {
        const l = ie(r);
        if (!l) continue;
        t[o].setAttribute("src", `data:${s};base64,${l}`);
      } catch {
      }
  }
  return a;
...
function re(i) {
  return oe(i).map((e, t) => {
    try {
      const n = atob(ie(e.hex)), a = new Uint8Array(n.length);
      for (let r = 0; r < n.length; r += 1)
        a[r] = n.charCodeAt(r);
      const o = e.type === "image/jpeg" ? "jpg" : "png";
      return new File([a], `rtf-${t}.${o}`, { type: e.type });
    } catch {
      return null;
    }
  }).filter(Boolean);
...
async function je(i) {
  ...
  if (e) return e;
  const t = Array.from(i.items || []).find(
    (a) => a.type === "text/rtf" || a.type === "application/rtf"
  );
  if (t && (e = await new Promise((a) => {
    t.getAsString((o) => a(o || ""));
  }), e))
    return e;
  if ((n = navigator.clipboard) != null && n.read)
    try {
      const a = await navigator.clipboard.read();
      for (const o of a)
        for (const r of ["text/rtf", "application/rtf"]) {
          if (!o.types.includes(r)) continue;
          const l = await (await o.getType(r)).text();
          if (l) return l;
        }
    } catch {
    }
...
function Qe(i) {
  ...
  for (; (t = n.exec(i)) !== null; ) {
    const o = t[1].match(/<v:shape[^>]*style="([^"]*)"[^>]*>/i), r = (o == null ? void 0 : o[1]) || "", s = r.match(/width:\s*([0-9.]+)(pt|px)[^;]*;\s*height:\s*([0-9.]+)(pt|px)/i) || r.match(/width:\s*([0-9.]+)(pt|px)/i);
    e.push({
      width: s ? Z(s[1], s[2]) : null,
      height: s && s[3] ? Z(s[3], s[4] || s[2]) : null
    });
  }
  return e;
}
function Ze(i, e, t, n, a = []) {
  ...
  for (let c = n; c < t.length; c += 1) {
    ...
    p.style.float = "right", p.style.margin = "0 0 12px 12px";
    ...
    b.style.overflow = "hidden", b.appendChild(p), i.firstChild ? i.insertBefore(b, i.firstChild) : i.appendChild(b), s.push({
      ...
      meta: {
        ...
        widthAttr: "",
        heightAttr: "",
        style: `width:${u.width};height:auto;float:right;margin:0 0 12px 12px;`,
        className: "",
        dimensions: u
      }
    });
...
function se(i) {
  return i.options.readOnly || i.options.pasteImageUpload === !1 ? !1 : (i.options.pasteImageUpload === !0, !!(i.options.imageUploadUrl || i.options.imageUploadHandler));
}
function K(...i) {
  const e = [], t = /* @__PURE__ */ new Set();
  return i.flat().forEach((n) => {
    var o;
    if (!n || !((o = n.type) != null && o.startsWith("image/"))) return;
    const a = `${n.name}|${n.size}|${n.type}|${n.lastModified}`;
...
function le(i) {
  ...
  if ((a = i.files) != null && a.length)
    for (const s of i.files)
      n(s);
  if ((o = i.items) != null && o.length)
  ...
  try {
    ...
    for (const n of e)
      for (const a of n.types) {
        if (!a.startsWith("image/")) continue;
        const o = await n.getType(a), r = H(o.type || a);
        t.push(new File([o], `clipboard-api-${t.length}.${r}`, {
          type: o.type || a
        }));
      }
    return t;
...
function ce(i) {
  var a;
  if (!i) return [];
  const e = [], t = /data:image\/(?:png|jpe?g|gif|webp|svg\+xml);base64,[A-Za-z0-9+/=\s]+/gi;
  let n;
  for (; (n = t.exec(i)) !== null; ) {
    const o = n[0].replace(/\s+/g, ""), r = ((a = o.match(/^data:([^;,]+)/)) == null ? void 0 : a[1]) || "image/png", s = B(o, `raw-${e.length}.${H(r)}`);
    s && e.push(s);
  }
  return e;
}
function tt(i) {
  ...
  const a = i.match(/<!--\[if gte vml 1\][\s\S]*?<!\[endif\]-->/gi) || [];
  return n = Math.max(n, a.length), n;
}
function de(i) {
  var r;
  if (!i) return [];
  ...
  let o;
  for (; (o = a.exec(i)) !== null; ) {
    const s = ((r = o[1].match(/^data:([^;,]+)/)) == null ? void 0 : r[1]) || "image/png", l = B(o[1], `vml-${e.length}.${H(s)}`);
    l && e.push(l);
  }
  return e;
}
...
async function ot(i, e) {
  try {
    ...
    const a = H(n.type);
    return new File([n], e, { type: n.type || `image/${a}` });
  } catch {
    return null;
  }
}
...
function z(i) {
  ...
  if (!e) return null;
  if (/^\d+$/.test(e)) return `${e}px`;
  if (/^\d+(\.\d+)?(px|em|rem|%)$/i.test(e)) return e;
...
function V(i, e) {
  if (i.nodeType === Node.TEXT_NODE)
    return e.createTextNode(i.textContent);
  ...
  const n = e.createElement(t.toLowerCase());
  if (i.getAttribute("style") && n.setAttribute("style", i.getAttribute("style")), t === "A") {
    const a = i.getAttribute("href") || "";
    a && !/^javascript:/i.test(a) && n.setAttribute("href", a), i.getAttribute("target") && n.setAttribute("target", i.getAttribute("target"));
  }
  if ((t === "TD" || t === "TH") && ["colspan", "rowspan", "width", "height", "align", "valign"].forEach((a) => {
    i.getAttribute(a) && n.setAttribute(a, i.getAttribute(a));
  }), t === "TABLE" && ["border", "cellpadding", "cellspacing", "width"].forEach((a) => {
    i.getAttribute(a) && n.setAttribute(a, i.getAttribute(a));
  }), i.childNodes.forEach((a) => {
    const o = V(a, e);
    o && (o.nodeType, Node.DOCUMENT_FRAGMENT_NODE, n.appendChild(o));
  }), (t === "O:P" || t === "W:SDT") && n.childNodes.length) {
    const a = e.createDocumentFragment();
    for (; n.firstChild; )
...
async function ct(i, e, t, n) {
  ...
  if (e.clipboardFiles[t.clipboard]) {
    const r = e.clipboardFiles[t.clipboard];
    return t.clipboard += 1, t.total += 1, { file: r, keepOriginal: !1 };
  }
  if (e.htmlDataFiles[t.htmlData]) {
    const r = e.htmlDataFiles[t.htmlData];
    return t.htmlData += 1, t.total += 1, { file: r, keepOriginal: !1 };
  }
  if (e.rtfFiles[t.rtf]) {
    const r = e.rtfFiles[t.rtf];
    return t.rtf += 1, t.total += 1, { file: r, keepOriginal: !1 };
  }
  return t.total += 1, { file: null, keepOriginal: !1 };
}
function $(i, e, t, n) {
  const a = i.editable.ownerDocument.createElement("span");
  a.className = "aracode-paste-image", a.dataset.pasteId = e, a.setAttribute("contenteditable", "false"), a.style.width = t.width, a.style.height = t.height, a.style.display = t.display;
  const o = i.editable.ownerDocument.createElement("span");
  o.className = "aracode-paste-image__spinner", o.setAttribute("aria-hidden", "true");
  const r = i.editable.ownerDocument.createElement("span");
  return r.className = "aracode-paste-image__label", r.textContent = n, a.appendChild(o), a.appendChild(r), a;
}
function he(i, e, t, n) {
  const a = $(i, e, t, n);
  a.classList.add("aracode-paste-image--error");
...
function W(i, e) {
  const t = e.createDocumentFragment(), n = String(i || "").split(/\r?\n/);
...
function F(i, e) {
  ...
  i.deleteContents();
  const t = e.lastChild;
  i.insertNode(e), t && (i.setStartAfter(t), i.collapse(!0));
  const n = window.getSelection();
...
function ee(i, e, t) {
  if (!(e != null && e.parentNode)) return;
  const n = e.dataset.pasteId || O(), a = {
    width: e.style.width || "120px",
    height: e.style.height || "80px",
    display: e.style.display || "inline-block"
  }, o = he(i, n, a, t);
  e.parentNode.replaceChild(o, e);
}
async function j(i, e, t, n) {
  const a = i.editable.ownerDocument;
  if (!e) {
    ee(i, t, g("pasteImageMissing", i.options.locale));
    return;
  }
  try {
    const o = await i._uploadImageFile(e);
    if (!o)
    ...
    console.error("Error al pegar imagen", o), ee(
      i,
      t,
      g("pasteImageError", i.options.locale)
    );
  }
}
...
function ue(i, e, t) {
  const n = i.options.locale, a = { width: "120px", height: "80px", display: "inline-block" }, o = $(
    i,
    O(),
    a,
    g("pasteImageUploading", n)
  );
  t.deleteContents(), t.insertNode(o), t.setStartAfter(o), t.collapse(!0);
  const r = window.getSelection();
  return r.removeAllRanges(), r.addRange(t), j(i, e, o, {
    alt: "",
    widthAttr: "",
    heightAttr: "",
    style: "",
...
function bt(i) {
  i.editable.addEventListener("paste", (e) => {
    ...
    if (!t) return;
    const n = t.getData("text/html"), a = t.getData("text/plain"), o = le(t), r = re(
      t.getData("text/rtf") || t.getData("application/rtf") || ""
    ), s = K(
      de(n || ""),
      ce(n || "")
    ), l = tt(n || "");
...
class yt {
  ...
  saveToolbarSelection() {
    ...
    const t = e.getRangeAt(0);
    if (!this.editable.contains(t.commonAncestorContainer)) {
      this._toolbarSavedRange = null;
      return;
    }
    this._toolbarSavedRange = t.cloneRange();
  }
  consumeToolbarSelection() {
    const e = this._toolbarSavedRange;
    return this._toolbarSavedRange = null, e || null;
  }
  restoreToolbarSelection(e) {
    if (!e) return;
    const t = window.getSelection();
    t.removeAllRanges(), t.addRange(e);
  }
  _ensureFontPreconnect() {
    if (typeof document > "u" || document.querySelector("link[data-aracode-fonts-preconnect]")) return;
    const e = "data-aracode-fonts-preconnect", t = document.createElement("link");
    t.rel = "preconnect", t.href = "https://fonts.googleapis.com", t.setAttribute(e, ""), document.head.appendChild(t);
    const n = document.createElement("link");
    n.rel = "preconnect", n.href = "https://fonts.gstatic.com", n.crossOrigin = "anonymous", n.setAttribute(e, ""), document.head.appendChild(n);
  }
  _bindEvents() {
  ...
  off(e, t) {
    return this._events[e] ? (this._events[e] = this._events[e].filter((n) => n !== t), this) : this;
  }
  emit(e, ...t) {
    e === "change" && this._syncUploadedImages();
    const n = this._events[e];
    return n && n.forEach((a) => a(...t)), this;
  }
  getHTML() {
  ...
  _getExportFontImports(e) {
    const t = /* @__PURE__ */ new Set(), n = document.createElement("div");
    return n.innerHTML = e, n.querySelectorAll('[style*="font-family"]').forEach((a) => {
      const o = a.style.fontFamily.split(",")[0].replace(/["']/g, "").trim();
      o && t.add(o);
    }), Array.from(t).filter((a) => this._loadedFonts.has(a)).map((a) => `@import url("https://fonts.googleapis.com/css2?family=${a.replace(/\s+/g, "+")}&display=swap");`).join(`
`);
  }
  _getExportCSS() {
    return `
...
.aracode-export-content::after {
  ...
  openLinkDialog() {
    ...
    this.dialog.open(
      g("linkDialogTitle", this.options.locale),
      [
        { label: g("linkDialogUrl", this.options.locale), value: "https://", autofocus: !0 },
        { label: g("linkDialogText", this.options.locale), value: t },
        { label: g("linkDialogTarget", this.options.locale), type: "checkbox", value: !0 }
      ],
      ([n, a, o]) => {
        n && n !== "https://" && this.commands.createLink(n, a, o ? "_blank" : "");
      }
    );
  }
  openImageDialog() {
    ...
    let t = null;
    const n = window.getSelection();
    n.rangeCount > 0 && (t = n.getRangeAt(0)), this.dialog.options.uploadText = g("imageUploadBtn", e), this.dialog.options.onUpload = async (r, s) => {
      const l = r.upload;
      if (l && l.file)
        try {
      ...
      [
        {
          ...
          tabs: [
            {
              label: g("imageUploadTab", e),
              fields: [
                {
                  type: "file",
                  name: "upload",
                  accept: "image/*",
                  placeholder: g("imageDropHint", e)
                }
              ]
            },
            {
              label: g("imageUrlTab", e),
              fields: [
                { label: g("imageDialogUrl", e), value: "", name: "url" },
                { type: "image-preview", name: "preview" },
                { label: g("imageDialogAlt", e), value: "", name: "alt" },
                {
                  type: "row",
                  fields: [
                    { label: g("imageWidth", e), value: "", name: "width", placeholder: "auto" },
                    { label: g("imageHeight", e), value: "", name: "height", placeholder: "auto" }
                  ]
                },
    ...
    const a = this.dialog.dialog.querySelector('[name="url"]'), o = this.dialog.dialog.querySelector(".aracode-dialog-url-preview");
    a && o && a.addEventListener("input", () => {
      o.src = a.value, o.style.display = a.value ? "block" : "none";
    });
  }
  async _uploadImageFile(e, t = null, n = "") {
    ...
    const s = await fetch(this.options.imageUploadUrl, {
      method: "POST",
      headers: r,
      body: a,
    ...
    if (!s.ok)
      throw new Error(l.error || l.message || g("imageUploadError", this.options.locale));
    return t && t.updateProgress(100), this._extractUploadUrl(l);
  }
  _extractUploadUrl(e) {
    if (!e) return null;
    ...
    if (e.data) {
      ...
      if (e.data.path) return e.data.path;
    }
    return null;
  }
  _trackUploadedImage(e, t) {
    const n = this._normalizeImageUrl(e);
    if (!n || !t) return;
    const a = this._uploadedImages.get(n) || {};
    this._uploadedImages.set(n, {
      ...a,
      url: n,
      file: t,
      deleted: !1,
      reuploading: !1,
      deleteTimer: a.deleteTimer || null
    });
  }
  _syncUploadedImages() {
    ...
    this._uploadedImages.forEach((t, n) => {
      if (e.has(n)) {
        t.deleteTimer && (clearTimeout(t.deleteTimer), t.deleteTimer = null), t.deleted && !t.reuploading && this._restoreDeletedImage(n);
        return;
      }
      !t.deleted && !t.deleteTimer && (t.deleteTimer = setTimeout(() => {
        t.deleteTimer = null, this._deleteUploadedImage(n);
  ...
  async _deleteUploadedImage(e) {
    ...
    n.append("action", "delete"), n.append("url", e);
    try {
      ...
      console.error("Error al eliminar la imagen", a);
    }
  }
  async _restoreDeletedImage(e) {
    ...
    if (!(!t || !t.file || t.reuploading)) {
      t.reuploading = !0;
      try {
        const n = await this._uploadImageFile(t.file, null, e), a = this._normalizeImageUrl(n || e);
        t.deleted = !1, t.reuploading = !1, a && a !== e ? (this._uploadedImages.delete(e), this._uploadedImages.set(a, t), this.editable.querySelectorAll("img[src]").forEach((o) => {
          this._normalizeImageUrl(o.getAttribute("src")) === e && o.setAttribute("src", a);
        })) : this.editable.querySelectorAll("img[src]").forEach((o) => {
          this._normalizeImageUrl(o.getAttribute("src")) === e && (o.setAttribute("src", ""), o.setAttribute("src", e));
        });
      } catch (n) {
        t.reuploading = !1, console.error("Error al restaurar la imagen", n);
      }
    }
  }
  ...
  loadFontFamily(e) {
    ...
    const t = document.createElement("link"), n = e.trim().replace(/\s+/g, "+");
    t.rel = "stylesheet", t.href = `https://fonts.googleapis.com/css2?family=${n}&display=swap`, document.head.appendChild(t), this._loadedFonts.add(e);
  }
  toggleSearchPanel() {
  ...
  openSearchPanel() {
    ...
    n.className = "aracode-search-input", n.type = "search", n.placeholder = g("searchPlaceholder", this.options.locale);
    const a = document.createElement("button");
    a.className = "aracode-search-close", a.type = "button", a.textContent = "x", a.setAttribute("aria-label", "Cerrar"), t.appendChild(n), t.appendChild(a);
    const o = document.createElement("div");
    o.className = "aracode-search-count";
    const r = document.createElement("div");
  ...
  _selectSearchMatch(e) {
    ...
    if (!t) return;
    const n = window.getSelection();
    n.removeAllRanges(), n.addRange(t), e.node.scrollIntoView({ behavior: "smooth", block: "center" }), this.editable.focus(), this._searchPanel && (this._searchPanel.remove(), this._searchPanel = null);
  }
  ...
  openColorPicker(e, t) {
    var o, r, s;
    if ((r = (o = this.toolbar) == null ? void 0 : o.tablePicker) == null || r.close(), (s = this.colorPicker) != null && s.isOpenFor(e)) {
      this.colorPicker.destroy(), this.colorPicker = null;
      return;
    }
    this.colorPicker && this.colorPicker.destroy(), this._searchPanel && (this._searchPanel.remove(), this._searchPanel = null);
    const n = window.getSelection();
    let a = null;
    if (n.rangeCount > 0) {
  ...
  toggleSourceView() {
    if (this._isSourceView = !this._isSourceView, this._isSourceView) {
      const e = this.getHTML();
      this._sourceTextarea = document.createElement("textarea"), this._sourceTextarea.className = "aracode-source", this._sourceTextarea.value = e, this._sourceTextarea.style.minHeight = `${this.options.height}px`, this._sourceTextarea.setAttribute("aria-label", g("sourceView", this.options.locale)), this.editable.style.display = "none", this.editable.parentNode.insertBefore(this._sourceTextarea, this.editable.nextSibling), this._sourceTextarea.focus();
    } else
  ...
  destroy() {
    this.emit("destroy"), this._events = {}, this._sourceTextarea && this._sourceTextarea.parentNode && this._sourceTextarea.parentNode.removeChild(this._sourceTextarea), this.toolbar.destroy(), this.dialog.destroy(), this.tableController && this.tableController.destroy(), this.colorPicker && this.colorPicker.destroy(), this._searchPanel && (this._searchPanel.remove(), this._searchPanel = null), this._uploadedImages.forEach((e) => {
